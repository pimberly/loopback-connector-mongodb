...
function MongoDB(settings, dataSource) {
  ...
  if (
    ...
    debug(
      'Optimized findOrCreate is enabled by default, ' +
  ...
  if (self.db) {
    ...
    if (self.settings.url && self.settings.database) {
      if (self.settings.url.indexOf('/' + self.settings.database) === -1) {
        self.settings.url = processMongoDBURL(
    ...
    callbackConnect(function(err, client) {
      if (err) {
        onError(err);
        return;
      }
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        if (command === 'findOneAndUpdate') {
          ensureFindOneAndUpdateResultMetadataResponse(args);
        }
  ...
  for (const propName in model.properties) {
    ...
    if (direction === 'database') {
      ...
      if (Array.isArray(data)) {
        ...
        // Handle data as Object - in case to create / update
 ...
 * @param {Function} [callback] The callback function
 */
MongoDB.prototype.findOrCreate = function findOrCreate(
  modelName,
  filter,
  data,
  options,
  ...
  const callbackFindOneAndUpdate = util.callbackify(() =>
  ...
  callbackFindOneAndUpdate(function(err, result) {
    if (self.debug) {
      ...
      function(modelName, modelCallback) {
        ...
        async.eachSeries(
          ...
          function(index, indexCallback) {
            ...
            const callbackCreateIndex = util.callbackify(() =>
              self
                .collection(modelName)
                .createIndex(index.fields || index.keys, index.options));
  ...
  if (self.db) {
    ...
    async.eachSeries(
      ...
      function(modelName, modelCallback) {
        ...
        const callbackDropCollection = util.callbackify((collectionName) =>
        ...
        callbackDropCollection(collectionName, function(err, collection) {
          ...
          const callbackCreateCollection = util.callbackify((collectionName) =>
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  if (
    (typeof value === 'string' && value.match(ObjectIdValueRegex)) ||
    (Array.isArray(value) &&
      value.every((v) => typeof v === 'string' && v.match(ObjectIdValueRegex)))
  ) {
...
function buildOptions(requiredOptions, connectorOptions) {
  if (
    connectorOptions &&
    connectorOptions.transaction &&
    connectorOptions.transaction.isActive()
  ) {
...
/**
 * MongoDB Node.js driver 4+ defaults findOneAndUpdate to return the document only.
 * This connector still expects the findAndModify reply: { value, lastErrorObject, ok }.
 *
 * @param {Array} args [filter, update, options?] (callback already removed)
 */
function ensureFindOneAndUpdateResultMetadataResponse(args) {
  if (args.length === 2) {
    args.push({includeResultMetadata: true});
    return;
  }
  const opts = args[2];
  if (opts && typeof opts === 'object') {
    args[2] = Object.assign({includeResultMetadata: true}, opts);
  } else {
...
function processMongoDBURL(settingsDatabase, mongoUrl) {
  ...
  if (mongoUrl) {
    // 1. Know protocol
    let baseUrl = '';
    if (mongoUrl.startsWith('mongodb:')) baseUrl = 'mongodb://';
    else if (mongoUrl.startsWith('mongodb+srv:')) baseUrl = 'mongodb+srv://';
    ...
    else return mongoUrl; // Not a MongoURL that we can process
    ...
    if (remainderUrl.indexOf('@') !== -1) {
      ...
      uidPassword = parts[0];
      if (parts.length === 2) remainderUrl = parts[1];
      else remainderUrl = '';
    }
    let hosts = '';
    let dbName = '';
    ...
    if (slashIndex !== -1) {
      ...
      if (hostsArray.length > 1) hosts = hostsArray.join(',');
      else hosts = remainderUrl;
    }
    ...
    if (uidPassword) modifiedUrl += uidPassword + '@';
    if (hosts) modifiedUrl += hosts;

    modifiedUrl += '/' + (dbName ? dbName : settingsDatabase);

    if (options) modifiedUrl += '?' + options;
