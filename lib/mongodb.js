...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
    ...
    const client = new mongodb.MongoClient(self.settings.url, {...validOptions, monitorCommands: true});

    client.on('commandStarted', event => {
      const collection = event.command[event.commandName];
      const trace = event.command['comment'] || 'notraceid';
      let detail = event;
      if (event.commandName === 'find') {
        detail = event.command['filter'];
      } else if (event.commandName === 'update') {
        detail = event.command['updates'];
      }
      console.log(`[MongoDB] ${trace} ${event.commandName} ${collection} ${JSON.stringify(detail)}`);
    });
